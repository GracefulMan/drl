...
class AgentPPO:
    ...
    def init(self, net_dim, state_dim, action_dim, if_use_gae=False):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.compute_reward = self.compute_reward_gae if if_use_gae else self.compute_reward_raw
...
def demo():
    ...
    if if_train_cart_pole:
        ...
        args.net_dim = 2 ** 7
        args.batch_size = args.net_dim * 2
        args.target_step = 2 ** 12 # args.env.max_step * 16
        args.if_allow_break = False
        args.break_step = int(4e5)
        args.gpu_id = 1
